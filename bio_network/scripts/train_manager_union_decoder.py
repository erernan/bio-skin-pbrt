...
def train(parameters):
    ...
    for run in run_builder.get_runs(parameters):
        ...
        a2s_net = torch.load("/data/wq/biological_smoother/model/union_net_a2s_23.pt").to(device)
        unet = torch.load("/data/wq/biological_smoother/model/union_net_unet_23.pt").to(device)
        decoder = torch.load("/data/wq/biological_smoother/model/decoder_26.pt").to(device)
        ...
        optimizer = torch.optim.Adam([
            ...
            {'params': decoder.parameters(), 'lr': 0.001, 'weight_decay':1e-4},
	        ])
        # tensorboard
        ...
        for epoch in trange(epoch):
            ...
            for batch in train_loader:
                ...
                preds_unet_decoer = torch.stack((preds_unet[:,0].contiguous().view(-1),preds_unet[:,1].contiguous().view(-1),preds_unet[:,2].contiguous().view(-1)),dim=1)

                preds_decoder = reshape_tensor(decoder(preds_unet_decoer).view(-1,256,256,3))


                # MLP l1 loss
                loss_mlp_melanin = 0.05 * F.l1_loss(preds_a2s[:,0],gt_cms)
                loss_mlp_hemoglobin = 0.05 * F.l1_loss(preds_a2s[:,1],gt_chs)
                loss_mlp_beta = 0.05 * F.l1_loss(preds_a2s[:,2],gt_betas)
                ...
                # decoder loss
                loss_decoder = 0.2 * F.l1_loss(preds_decoder,albedos)

                loss_melanin += loss_mlp_melanin
                loss_hemoglobin += loss_mlp_hemoglobin
                loss_beta += loss_mlp_beta

                loss = loss_melanin + loss_hemoglobin + loss_beta + loss_decoder
